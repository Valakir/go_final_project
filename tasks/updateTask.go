...
func UpdateTaskHandler(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	if _, err := time.Parse(dates.DateFormat, task.Date); err != nil {
		RespondWithError(w, http.StatusBadRequest, `{"error":"Некорректная дата"}`)
		return
	}

	// Проверяем что дата не в прошлом
	if task.Date < time.Now().Format(dates.DateFormat) {
		RespondWithError(w, http.StatusBadRequest, `{"error":"Дата не может быть меньше сегодняшней"}`)
		return
	}

	// Проверяем что заголовок заполнен
	if task.Title == "" {
		RespondWithError(w, http.StatusBadRequest, `{"error":"Заголовок не может быть пустым"}`)
		return
	}
	...
	if err != nil {
		RespondWithError(w, http.StatusInternalServerError, fmt.Sprintf(`{"error":"Ошибка обновления задачи: %s"}`, err.Error()))
		return
	}

	rowsAffected, err := result.RowsAffected()
	if err != nil {
		RespondWithError(w, http.StatusInternalServerError, fmt.Sprintf(`{"error":"Ошибка получения результата обновления: %s"}`, err.Error()))
		return
	}
